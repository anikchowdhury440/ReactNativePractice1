import React, { useEffect, useState } from 'react'
import {View, Button, Image, Linking} from 'react-native'
// import ImagePicker from 'react-native-image-crop-picker';
// import RNFS from 'react-native-fs';
import FastImage from 'react-native-fast-image'
import ImageView from "react-native-image-viewing";

const url = 'https://homework-automation.s3.amazonaws.com/1/PrePrimary/9999/WS_2.jpeg?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=AKIAQWJOGETZ4ARZMYXV%2F20210311%2Fap-south-1%2Fs3%2Faws4_request&X-Amz-Date=20210311T092322Z&X-Amz-Expires=3600&X-Amz-SignedHeaders=host&X-Amz-Signature=15479e2ec0e6a68b3e94668bd6a48d6c604e39b02a3b0166ab0829cddc0a9e5c'
const url1 = 'data:image/jpeg;base64,'
const url2 = 'https://reactnative.dev/img/tiny_logo.png'

import ImgToBase64 from 'react-native-image-base64';
// import RNFetchBlob from "rn-fetch-blob";
// const fs = RNFetchBlob.fs;
// let imagePath = null;

const ImagePick = () => {
    const [image, setImage] = useState('')

    const handlePress = async () => {
        // ImagePicker.openCamera({
        //     width: 300,
        //     height: 400,
        //     cropping: true,
            
        //   }).then(image => {
        //     console.log(image);
        //   });
    
        console.log(true)
        // const fileReader = new FileReader();
        // fileReader.onload = fileLoadedEvent => {
        //     const base64Image = fileLoadedEvent.target.result;
        // };
        // fileReader.readAsDataURL(url); 
        ImgToBase64.getBase64String('https://homework-automation.s3.amazonaws.com/1/PrePrimary/9999/WS_13.jpeg?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=AKIAQWJOGETZ4ARZMYXV%2F20210311%2Fap-south-1%2Fs3%2Faws4_request&X-Amz-Date=20210311T120553Z&X-Amz-Expires=3600&X-Amz-SignedHeaders=host&X-Amz-Signature=f5aaa2407a5070d0b5dc357111c96d93edfe733cd80d44e741b54b257adf6f59')
            .then(base64String => console.log(base64String))
            .catch(err => console.log(err));
        // RNFetchBlob.config({
        //     fileCache: true
        //   })
        //     .fetch("GET", url2)
        //     // the image is now dowloaded to device's storage
        //     .then(resp => {
        //         console.log(resp)
        //       // the image path you can use it directly with Image component
        //       imagePath = resp.path();
        //       return resp.readFile("base64");
        //     })
        //     .then(base64Data => {
        //       // here's base64 encoded image
        //       console.log(base64Data);
        //       // remove the file from storage
        //       return fs.unlink(imagePath);
        //     })
        //     .catch(error => console.log('Errorwa', error))
        // RNFS.readFile(url, 'base64')
        //     .then(res =>{
        //         console.log(res);
        //     }).catch(error => console.log(error));
    }
    return(
        <View style = {{flex : 1, justifyContent : 'center', alignItems : 'center'}}>
            <Button title = 'Choose Photo' onPress = {handlePress}/>
            {/* <Image source = {{uri : 'https://reactnative.dev/img/tiny_logo.png'}} style = {{height : 100, width : 100}} /> */}
            <Image source={{uri: 'https://reactjs.org/logo-og.png'}}
       style={{width: 400, height: 400}} />
        </View>
    )
}

export default ImagePick